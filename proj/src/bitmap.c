#include "bitmap.h"
 ...
 // Added by SnakeX 3 authors to remove implicit declaration compiler warnings.
#include <stdlib.h>
#include <string.h>
#include "video_gr.h"
...
void drawBitmap(Bitmap* bmp, int x, int y, Alignment alignment) {
    ...
    if (x + width < 0 || x > getHorRes() || y + height < 0
            || y > getVertRes()) {
    	printf("drawBitmap(): invalid x, y, width or height\n");
    	return;
    }
    ...
    for (i = 0; i < height; i++) {
        ...
        if (pos < 0 || pos >= getVertRes())
            continue;

        bufferStartPos = getSecBuffer();
