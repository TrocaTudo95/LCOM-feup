...
int timer_subscribe_int(void) {
	if (isSubscribed) {
		printf("Subscribing already-subscribed timer\n");
		return -1;
	}
	...
	if (sys_irqenable(&timer_hook_id) != OK)
		return -1;
	isSubscribed = true;
	timerSubResult = temp_hook_id;
	return temp_hook_id;
}

int timer_unsubscribe_int() {
	if (!isSubscribed) {
		printf("Unsubscribing already-unsubscribed timer\n");
		return -1;
	}
	int temp_hook_id = timer_hook_id;
	timer_hook_id = 1;
...
int waitByTimer0(double n) {
	...
	unsigned long irq_timer_set;
	if (!wasSubscribed) {
		irq_timer_set = BIT(timer_subscribe_int());
	} else {
		irq_timer_set = BIT(timerSubResult);
	}
